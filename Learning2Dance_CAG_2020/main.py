...
def train_gcn(args, device):
    ...
    if args.ckp_save_path is None:
        log_summary = '/tmp/summary/'
    else:
        log_summary = args.ckp_save_path
    gen_checkpoint_path = args.ckp_save_path + '_generator.pt'
    dis_checkpoint_path = args.ckp_save_path + '_discriminator.pt'
    try:
        os.makedirs(args.ckp_save_path, exist_ok=True)
    ...
    for epoch in range(args.epochs):
        ...
        ##########DRAWWING WEIGHTS##########
        draw_weights('generator_', generator_network, None, epoch)
        draw_weights('discriminator_', discriminator, None, epoch)
        ####################################
...
def test(args, device):
    ...
    os.makedirs(args.out_video + video_name + '/vid2vid/test_img/', exist_ok=True)
    make_video(args.out_video + video_name + '/vid2vid/test_img/', notorch_pose, fps=30)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, default='dataset', help='dataset path')
    parser.add_argument('--a_ckp_path', type=str, default='checkpoints/audio.pt', help='path to audio model')
    parser.add_argument('--ckp_path', type=str, default='checkpoints/generator.pt', help='path to generator model')
    ...
    parser.add_argument('--size_video', type=int, default=16, help='number of frames in video')
    parser.add_argument('--size_sample', type=int, default=128, help='size of samples')
    ...
    parser.add_argument('--lambda_l1', type=float, default=0.5, help='lambda for l1 loss')
    parser.add_argument('--lambda_discriminator', type=float, default=0.5, help='lambda for discriminator loss')
    parser.add_argument('--lambda_l2', type=float, default=0.5, help='lambda for l2 loss')
    parser.add_argument('--dropout', type=float, default=0.0, help='dropout rate')
    parser.add_argument('--workers', type=int, default=4, help='number of workers for dataloader')
    parser.add_argument('--flip', type=float, default=0.0, help='probability of flipping')
    parser.add_argument('--adam', action='store_true', help='use Adam optimizer')
    parser.add_argument('--mse', action='store_true', help='use MSE loss instead of BCE loss')
    parser.add_argument('--ckp_save_path', type=str, help='checkpoint save path', required=True)
